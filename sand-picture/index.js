...
(function () {
    ...
    viscosityRange.addEventListener('input', () => {
        viscVal.textContent = Number(viscosityRange.value).toFixed(2);
    });
    surfaceTensionRange.addEventListener('input', () => {
    ...
    densityLevelsRange.addEventListener('input', () => {
        densVal.textContent = densityLevelsRange.value;
    });
    if (sandColorInputA) sandColorInputA.addEventListener('input', () => {
        updatePaletteFromPicker();
    });
    if (sandColorInputB) sandColorInputB.addEventListener('input', () => {
        updatePaletteFromPicker();
    });
    ...
    function isSandType(t) {
        return t === TYPE.SAND_A || t === TYPE.SAND_B;
    }
